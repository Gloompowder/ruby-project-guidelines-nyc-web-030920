        ...
        while plant_choice != "Exit"
            ...
            if plant_choice == "New Plant🌱"
                ...
                if scientific_name == "" || scientific_name == " "
                    ...
                    id=JSON.parse(RestClient.get("#{api_beginning}#{api_end}#{my_key}&common_name=#{common_name}")).first["id"]
                else 
                    id=JSON.parse(RestClient.get("#{api_beginning}#{api_end}#{my_key}&scientific_name=#{scientific_name}")).first["id"]
                end 
                plant_info = JSON.parse(RestClient.get("#{api_beginning}#{api_end}#{id}#{my_key}"))
                plant = {} 
                plant["#{nickname}"] = Plant.create(nickname: nickname,
                common_name: JSON.parse(RestClient.get("#{api_beginning}#{api_end}#{id}#{my_key}&common_name=#{common_name}"))["common_name"],
